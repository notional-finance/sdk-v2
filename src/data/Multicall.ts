...
async function executeStage<T extends Record<string, any>>(
  ...
  const aggregateCall = calls.map((c) => {
    const contract = typeof c.target === 'function' ? c.target(aggregateResults) : c.target;
    const args = typeof c.args === 'function' ? c.args(aggregateResults) : c.args;
  ...
  const results = returnData.reduce((obj, r, i) => {
    ...
    (obj[key] as Record<string, any>) = transform ? transform(decoded, obj) : decoded;
    return obj;
  }, aggregateResults);
