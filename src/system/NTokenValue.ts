...
export default class NTokenValue {
  ...
  public static getNTokenStatus(currencyId: number, blockTime = getNowSeconds()) {
  ...
  public static getAssetFromRedeemNToken(
    ...
    return liquidityTokensToWithdraw.reduce((totalAssetCash, lt) => {
      ...
      if (!netfCashShare.isZero()) {
        ...
        const market = cashGroup.getMarket(CashGroup.getMarketIndexForMaturity(lt.maturity, blockTime));
  ...
  private static getTokensToWithdraw(
    ...
    const totalAssetValueInMarkets = liquidityTokens.reduce((total, lt) => {
      ...
      return total
        .add(assetCashClaim)
        .add(cashGroup.getfCashPresentValueUnderlyingInternal(lt.maturity, netfCash, false, blockTime).toAssetCash());
    ...
    const ifCashRiskAdjustedValue = fCash
      ...
      .reduce(
        (total, f) =>
          total.add(
            cashGroup.getfCashPresentValueUnderlyingInternal(f.maturity, f.notional, true, blockTime).toAssetCash()
          ),
        TypedBigNumber.from(0, BigNumberType.InternalAsset, currency.assetSymbol)
      );
