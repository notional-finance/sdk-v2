...
export default class Boosted3TokenAuraVault extends BaseBalancerStablePool<InitParams> {
  ...
  public async getRedeemParametersExact(
    ...
    const { amountRedeemed } = this.getRedeemGivenStrategyTokens(maturity, strategyTokens, blockTime);
    return {
      minPrimary: amountRedeemed.toExternalPrecision().scale(RATE_PRECISION - slippageBuffer, RATE_PRECISION).n,
