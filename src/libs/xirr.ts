...
export const calculateResult = (flowsFrom1: ReadonlyArray<CashFlowNormalized>, r: number): number =>
  flowsFrom1.reduce<number>((result, {date, amount}) => result + amount / Math.pow(r, date), 0.0);

export const calculateResultDerivation = (flowsFrom1: ReadonlyArray<CashFlowNormalized>, r: number): number =>
  flowsFrom1.reduce<number>((result, {date, amount}) => result - (date * amount) / Math.pow(r, date + 1.0), 0.0);
...
export const calculate = (
  ...
  do {
    ...
    do {
      resultValue = firstFlowAmount + calculateResult(flowsFrom1, resultRate + 1);
      const newRate: number = resultRate - resultValue / calculateResultDerivation(flowsFrom1, resultRate + 1);
    ...
    doLoop = doLoop || isNaN(resultRate) || !isFinite(resultRate) || isNaN(resultValue) || !isFinite(resultValue);
...
): number => calculate(normalize(flows), guessRate, maxEpsilon, maxScans, maxIterations);

export const convertBigNumber = (x: BigNumber) => Number(ethers.utils.formatUnits(x, 8));
/* eslint-enable */
